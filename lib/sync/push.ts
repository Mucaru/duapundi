...
async function updateLocalTimestamp(
  ...
  if (entity === "transaction") {
    ...
    await db.budgets.update(id, { updated_at: updatedAt });
  }
}
...
async function softDeleteOrphanLocal(entity: SyncEntity, id: string): Promise<void> {
  ...
  if (entity === "transaction") {
    ...
    await db.budgets.update(id, { deleted_at: now, updated_at: now });
  }
}
...
async function processItem(item: SyncQueueItem): Promise<boolean> {
  ...
  if (!table) {
    // household belum ditangani sync engine — di luar scope MVP saat ini.
    return true;
  }
